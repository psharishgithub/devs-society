...
// @access  Private
router.get('/event/:eventId/my-qr', auth_1.default, async (req, res) => {
    try {
        ...
        if (!qrCodeUrl || !checkInCode) {
            const qrData = await qrCodeService_1.default.generateEventQRCode(eventId, userId, registration.id);
...
// @access  Admin
router.post('/check-in', roleBasedAuth_1.adminAuth, [
    (0, express_validator_1.body)('qrCodeData').notEmpty().withMessage('QR code data is required'),
    ...
    try {
        ...
        const supabase = (0, supabase_1.getSupabase)();
        // Verify QR code data
        const qrData = qrCodeService_1.default.verifyQRCode(qrCodeData);
        if (!qrData) {
            return res.status(400).json({
                success: false,
...
// @access  Admin
router.post('/check-in-by-code', roleBasedAuth_1.adminAuth, [
    (0, express_validator_1.body)('checkInCode').trim().isLength({ min: 8 }).withMessage('Valid check-in code is required'),
    (0, express_validator_1.body)('notes').optional().isString()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
...
// @access  Admin
router.get('/event/:eventId/check-ins', roleBasedAuth_1.adminAuth, async (req, res) => {
    try {
        const { eventId } = req.params;
        const supabase = (0, supabase_1.getSupabase)();
...
// @access  Admin
router.get('/event/:eventId/scanner-qr', roleBasedAuth_1.adminAuth, async (req, res) => {
    try {
        const { eventId } = req.params;
        // Verify event exists
        ...
        const scannerQRCode = await qrCodeService_1.default.generateScannerQRCode(eventId);
        res.json({
            success: true,
            scannerQRCode,
            event: {
...
// @access  Admin
router.post('/verify-member', roleBasedAuth_1.adminAuth, [
    (0, express_validator_1.body)('qrCodeData').notEmpty().withMessage('QR code data is required'),
    (0, express_validator_1.body)('eventId').optional().isUUID().withMessage('Valid event ID is required if provided'),
    (0, express_validator_1.body)('notes').optional().isString()
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
...
// @access  Admin
router.post('/check-in-member', roleBasedAuth_1.adminAuth, [
    (0, express_validator_1.body)('qrCodeData').notEmpty().withMessage('QR code data is required'),
