...
exports.storageService = {
    ...
    uploadProfilePhoto: async (fileBuffer, fileName, userId) => {
        try {
    ...
    deleteProfilePhoto: async (photoUrl) => {
        try {
    ...
    updateProfilePhoto: async (fileBuffer, fileName, userId, oldPhotoUrl) => {
        try {
            // Delete old photo if it exists
            if (oldPhotoUrl) {
                await exports.storageService.deleteProfilePhoto(oldPhotoUrl);
            }
            // Upload new photo
            return await exports.storageService.uploadProfilePhoto(fileBuffer, fileName, userId);
        }
        catch (error) {
    ...
    uploadEventPhoto: async (fileBuffer, fileName, eventId) => {
        try {
            const fileExt = fileName.split('.').pop()?.toLowerCase();
            const allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
            if (!fileExt || !allowedExtensions.includes(fileExt)) {
