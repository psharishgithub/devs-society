...
async function testFixes() {
  try {
    ...
    try {
      const validation = await UserService.validateBatchYearAssignment('test-college-id', '2024')
    ...
    try {
      const availableBatches = await UserService.getAvailableBatchYears('test-college-id')
